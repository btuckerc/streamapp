...
python3 scripts/build-app.py --release \
  ...
  --version 1.3.0 --build-number 9 --output /absolute/path/StreamApp.app
```
...
**Settings → Audio → Processing → Echo cancellation** optionally reduces speaker playback entering your microphone, before microphone gain/compression. It is saved independently of compression and defaults off. It uses other apps' playback privately as a reference even when broadcast app audio is disabled, muted, or limited to one app; the reference itself is never added to your recording/stream. Your selected system-audio feed stays stereo and unchanged. Enabled AEC makes the microphone mono and can soften quiet speech or musical input; use bypass/headphones for critical music. The status shows when cancellation is active or bypassed because its reference is unavailable. This is in-app WebRTC processing, not macOS Voice Isolation.
